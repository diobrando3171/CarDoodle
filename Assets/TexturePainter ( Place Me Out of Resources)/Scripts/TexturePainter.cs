...
//FIXME:增加heart笔刷
public enum Painter_BrushMode{PAINT,DECAL,HEART};
public class TexturePainter : MonoBehaviour {
	...
	void DoAction(){	
		...
		if(HitTestUVPosition(ref uvWorldPosition)){
            ...
            else if(mode == Painter_BrushMode.DECAL){
				//brushObj=(GameObject)Instantiate(Resources.Load("TexturePainter-Instances/DecalEntity")); //Paint a decal
                brushObj = (GameObject)Instantiate(Resources.Load("Prefabs/DecalEntity")); //Paint a decal
            }
            else{
                //brushObj = (GameObject)Instantiate(Resources.Load("TexturePainter-Instances/HeartEntity"));
                brushObj = (GameObject)Instantiate(Resources.Load("/Resources/Prefabs/HeartEntity"));
            }
	...
	bool HitTestUVPosition(ref Vector3 uvWorldPosition){
		RaycastHit hit;
        //FIXME:排除鼠标在UI层上的操作 以及屏幕触控操作
        bool isOverUI = EventSystem.current.IsPointerOverGameObject();
        bool isTouching = CameraControlByTouch.IsTouching;
        if (isOverUI || isTouching) return false;
        //------------------------
    ...
    public void ClearDraw(){
        int countBrush = brushContainer.transform.childCount;
        if (brushContainer == null) return;
        //FIXME:此处不能用while循环 Destory有延迟 
        //while(brushContainer.transform.childCount>0){
        //    Destroy(brushContainer.transform.GetChild(0).gameObject);
        //}
        //
        //Destory 是有延迟的 对象会在这一帧结束后才会被清除 
